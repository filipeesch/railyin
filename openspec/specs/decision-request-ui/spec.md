...
The `DecisionInterviewPanel` SHALL provide a dismiss control that closes the panel without submitting answers or sending anything to the model. Dismissal SHALL be scoped to the current interview episode: it resets when a NEW interview episode begins (pages arrive from a different execution after dismissal), so the panel can spawn again for later questions.
...
- **WHEN** the user dismisses the panel and a new execution streams new question pages afterward
