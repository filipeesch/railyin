## Purpose
Defines how column-specific `stage_instructions` is delivered to the AI engine: via a `userContent`-layer `<active_directive>` block (never `systemInstructions`), so provider-side prefix/prompt caching on the invariant system prompt is preserved across column transitions, with an explicit binding-directive framing and cancellation block when no stage_instructions is configured for the current column.
