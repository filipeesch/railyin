## Purpose
Specifies the behaviour and test contract for `ContextEstimator`, which estimates token usage for a conversation via a fast path (last completed execution) or a slow path (compaction-anchored message scan).
