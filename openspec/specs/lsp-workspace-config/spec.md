## Purpose
LSP workspace config covers how LSP server configuration is scoped to specific workspaces: writing config to the correct workspace, resolving LSP servers per task's workspace, and ensuring the UI components carry and forward workspace context.
