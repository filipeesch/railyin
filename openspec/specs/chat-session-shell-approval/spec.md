## Purpose
Defines shell auto-approve state persistence and approval flow for workspace chat sessions, including per-session toggle, per-session approved commands, Claude and OpenCode engine support, and a unified shell approval response endpoint.
