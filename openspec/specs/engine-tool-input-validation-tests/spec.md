...
Defines the test suite requirements for `validateToolArgs` and the `executeCommonTool` validation gate. Complements the `engine-tool-input-validation` capability spec with concrete test coverage expectations.
