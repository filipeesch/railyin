...
## Purpose
Provides a dialect-agnostic utility for scanning project instruction files (`.github/instructions/` for Copilot, `.cursor/rules/` for Cursor), parsing YAML frontmatter, and returning structured `Instruction` objects for injection into engine system prompts.
