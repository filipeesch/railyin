## Purpose
Defines how users attach files (images, PDFs, text, and other file types) to chat messages, how attachments are staged before send, and how each AI engine adapter translates attachments into provider-specific formats.
...
**Copilot engine** (text/json/yaml → temp file + `{ type: "file", path }`, images/binary → `{ type: "blob", mimeType, data }`, first turn only):

#### Scenario: Text file attachment forwarded to Copilot via temp file
- **WHEN** a message includes a text/json/yaml file and the engine is Copilot
- **THEN** the content is written to a temp file and passed as `{ type: "file", path: "<tmpPath>", displayName: "<label>" }`

#### Scenario: Image/binary attachment forwarded to Copilot as blob
- **WHEN** a message includes an image or binary file and the engine is Copilot
