# Spec: Pi Tool Args Normalization

## Purpose

Ensures that when the Pi engine's AI models send tool call arguments as JSON-encoded strings (instead of native objects/arrays), those arguments are automatically normalized before SDK validation. This prevents tool call failures caused by models that serialize array or object arguments as JSON strings rather than passing them natively.
