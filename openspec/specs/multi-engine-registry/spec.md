## Purpose
Defines the `EngineRegistry` design for a multi-engine world: pre-constructed singleton instances keyed by engine ID, workspace-level `allowed_engines` filtering, and DI-based composition with no concrete engine imports in the registry itself.
