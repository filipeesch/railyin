...
When the user clicks a run profile button (or selects one from the dropdown) or clicks a tool button, the frontend SHALL call `launch.run` with the task ID and the command from that entry.
