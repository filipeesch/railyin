## Purpose
Defines how workflow columns can declare an explicit allowed-transitions list and how the UI and backend enforce that constraint.
...
The task drawer's workflow-state Select SHALL show only the current column (as a disabled option) and valid transition targets based on the current column's `allowedTransitions`, using the same data already available from the `boards.list` response. Columns that are not reachable SHALL be excluded from the Select options entirely. The current column is always present and disabled so the user can see the active state.
