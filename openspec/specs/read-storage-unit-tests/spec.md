...
Unit-test coverage for the `readStorage<T>(key, fallback)` localStorage utility, verifying correct JSON parsing, fallback behaviour on missing keys, malformed JSON, and environments where `localStorage` is unavailable.
