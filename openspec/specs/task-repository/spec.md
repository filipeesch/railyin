## Purpose
Defines the `TaskRepository` abstraction that provides a unified, JOIN-complete task read so all layers of the system receive consistent task objects — including git context fields — without needing to duplicate the LEFT JOIN logic.
