## Purpose
The `message_task` tool allows an AI agent to send a message to another task's conversation and trigger that task's AI model. If the target task is currently running, the message is queued and delivered after the task's execution ends.
