## Purpose
The refinement skill is a Copilot skill that automates the implement → measure → evaluate → iterate cycle for a given change. It drives the full refinement loop: collecting a baseline, implementing task groups with checkpoints, detecting regressions, and promoting across layers (mock → local → live).
