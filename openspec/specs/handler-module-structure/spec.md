## Purpose
Defines the structural requirements for how backend handler modules are organized, scoped, and composed. Ensures each handler file has a single domain responsibility, accepts only the dependencies it uses, and that supporting utilities are co-located with related modules.
