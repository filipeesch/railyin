# Spec: Worktree DI Test Coverage

## Purpose

Defines the test coverage requirements for the worktree subsystem after its dependency-injection refactor. Tests are structured by layer (repository, config resolver, git ops, manager, handler) and use in-memory DBs, stub interfaces, and real git repos to stay fast and isolated.
