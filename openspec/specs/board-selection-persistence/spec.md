## Purpose
Ensures the active workspace and active board selections survive page reloads by persisting them to `localStorage`. On startup the system restores the last-used selections, falling back gracefully when the saved values no longer exist.
