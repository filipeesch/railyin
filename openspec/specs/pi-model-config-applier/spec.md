# Pi Model Config Applier

## Purpose
Documents how the Pi engine resolves a model's per-model config (Mode variants, sampling presets, axes) into a canonical SDK thinking level and an `onPayload` request-body merge, and how the reasoning-effort key ownership is delegated to the pi SDK.
