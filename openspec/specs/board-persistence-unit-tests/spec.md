...
Pinia unit-test coverage for the board store's localStorage persistence behaviour: init-from-storage, watch-based write-back, stale-id fallback, and cross-workspace guard inside `loadBoards(workspaceKey?)`.
