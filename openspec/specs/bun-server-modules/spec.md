# Spec: bun-server-modules

## Purpose

Defines the server-side module decomposition extracted from `src/bun/index.ts`. Each module encapsulates a single responsibility (broadcast, notifications, stream processing, WebSocket lifecycle, shutdown, file logging) and `index.ts` is reduced to a linear composition root that wires them together.
