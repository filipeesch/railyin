...
### Requirement: Source engine resolved internally by CrossEngineContextInjector
The `CrossEngineContextInjector` SHALL internally resolve the source engine using `engineRegistry.getEngineById(conversations.last_engine_type)`. The `sourceEngine` parameter has been removed from `prepareSwitch()`; callers no longer need to look up or pass the source engine. This enables pre-switch compaction for Pi-to-other-engine switches when the chat context exceeds 75% of the target model's context window.
