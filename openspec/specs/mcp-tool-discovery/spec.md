## Purpose
Defines the three discovery/invocation common tools — `list_mcp_servers`, `list_mcp_tools`, and `invoke_mcp_tool` — that replace native per-tool MCP injection with a single, uniform discovery-based mechanism available identically across all engines (Copilot, Cursor, Claude, Pi, OpenCode) via `COMMON_TOOL_DEFINITIONS`/`executeCommonTool`. Instead of every enabled MCP tool being injected as its own native tool definition, the model discovers servers and tools on demand and invokes them through these generic tools, with visibility governed by the per-task/session `enabled_mcp_tools` override (see the `mcp-tool-injection` capability) and server resolution provided via `CommonToolContext.runtime.mcpRegistry`.
