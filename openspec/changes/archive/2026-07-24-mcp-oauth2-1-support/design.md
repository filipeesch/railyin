...
### 4a. `scope` request parameter resolved from discovery metadata (post-launch fix)
The initial implementation never sent a `scope` param on `/authorize`, leaving the choice entirely to the authorization server. Atlassian's Rovo MCP server issued a token that was missing scopes the protected resource actually required, surfacing as an opaque `"scope does not match"` error on the first authenticated tool call. A new pure function, `resolveAuthorizationScope()` (`src/bun/oauth/scope-resolution.ts`), picks the scope to request in priority order — mirroring the precedent set by `mcp-remote` (the reference client Atlassian's own docs point to): (1) the `scope="..."` attribute on the 401's `WWW-Authenticate` challenge, if present; (2) RFC9728 Protected Resource Metadata's `scopes_supported` (the resource-authoritative source — this is what Atlassian advertises); (3) RFC8414 Authorization Server Metadata's `scopes_supported` as a weaker fallback. Returns `undefined` (omitting the param) if none advertise anything, preserving prior behavior for servers that don't advertise scopes at all.
- *Alternative considered*: hardcoding scope resolution inline in `McpClientRegistry._discover()`/`authorize()` — rejected in favor of a small standalone, independently unit-testable pure function, keeping the registry focused on state-machine orchestration.
...
- **[Risk]** Redirect URI hardcodes `localhost:<port>`; if the app's port changes between DCR registration and token use (unlikely, but possible across restarts with `--port=`), redirect URI mismatch could break a stored client registration. → **Mitigation (implemented)**: `DcrClientRegistration` persists the `redirectUri` it was registered with; `McpClientRegistry._discover()` compares it against the current redirect URI on every discovery pass and discards + re-registers automatically on mismatch, so a stale cached client self-heals on the next `mcp.authorize` call instead of surfacing an opaque authorization-server error.
