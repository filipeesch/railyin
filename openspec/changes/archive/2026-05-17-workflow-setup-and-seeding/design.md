...
`workflow.list` returns each workflow as `{ id, name, boardCount, deletable, undeletableReason }`. The handler queries the `boards` table for counts per `workflow_template_id` (scoped to the workspace), and `evaluateDeletable` produces `deletable` + a human reason. Three guard conditions, in precedence order: the workflow is **bundled** (its id appears in the bundled source — deleting it is futile because seeding recreates it), it is **referenced by a board**, or it is the **last remaining** workflow. The UI disables the trash button purely from `deletable`. `workflow.delete` independently recomputes the same guard and throws on violation.
*Alternative considered:* frontend derives guards from the loaded board store — rejected: duplicates rule logic and couples the tab to the board store. *Bundled-guard alternative considered:* reworking seeding to be fresh-install-only so bundled workflows become deletable — rejected as a larger change to seeding semantics; making bundled workflows non-deletable eliminates the resurrection bug with a single guard condition.
