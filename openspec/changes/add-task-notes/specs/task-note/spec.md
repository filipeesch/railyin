...
`task_notes` SHALL have columns: `id` (INTEGER PK AUTOINCREMENT), `conversation_id` (INTEGER NOT NULL, FK to `conversations(id)` ON DELETE CASCADE), `title` (TEXT, nullable), `content` (TEXT NOT NULL), `is_source_ai` (INTEGER NOT NULL DEFAULT 0), `created_at` (TEXT NOT NULL DEFAULT datetime('now')), `updated_at` (TEXT NOT NULL DEFAULT datetime('now')).
...
- `createNote(conversationId, input: { title?: string; content: string; isSourceAi?: boolean }): TaskNote`
- `updateNote(id, input: { title?: string | null; content?: string }): TaskNote`
...
- **WHEN** `createNote(conversationId, { title: "AI scratch", content: "...", isSourceAi: true })` is called
- **THEN** the row has `is_source_ai = 1`

#### Scenario: updateNote patches provided fields only
- **WHEN** `updateNote(id, { content: "Updated content" })` is called without changing title
- **THEN** only `content` and `updated_at` are changed; `title` is preserved
...
- `"notes.create"` — params: `{ conversationId: number; title?: string; content: string }`, response: `TaskNote`
- `"notes.update"` — params: `{ noteId: number; title?: string | null; content?: string }`, response: `TaskNote`
- `"notes.delete"` — params: `{ noteId: number }`, response: `void`
...
`NotesPanel.vue` SHALL display all notes for the current `conversationId`, each showing its title (or a truncated content preview when title is absent) and `updated_at` timestamp. A **"+ New"** button SHALL open `NoteDetailOverlay.vue` for creating a new note. Clicking an existing note SHALL open the overlay in edit mode. A per-item delete button SHALL prompt for confirmation and hard-delete the note via `notes.delete`.

`NoteDetailOverlay.vue` SHALL provide a title input (optional) and a markdown textarea (required). It SHALL expose **Save** and **Cancel** actions; in edit mode it SHALL also expose a **Delete** action. The overlay SHALL call `notes.create` or `notes.update` as appropriate.
...
- **THEN** each note's title and timestamp are displayed in the panel
