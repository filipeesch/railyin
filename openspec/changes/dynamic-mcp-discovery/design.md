...
### D7 — `ExecutionParams.mcpRegistry`/`enabledMcpTools` are kept, not removed

**Superseded during implementation (decision recorded 2026-08-05):** rather than removing these fields and threading `McpRegistryPool` into every engine constructor, `ExecutionParams.mcpRegistry`/`enabledMcpTools` and `ExecutionParamsBuilder` remain exactly as they are today — the single place that resolves the project/global `McpClientRegistry` via the pool and parses `task.enabled_mcp_tools` into a `string[] | null`. Each engine's `CommonToolContext` construction copies `params.mcpRegistry` → `runtime.mcpRegistry` and `params.enabledMcpTools` → `runtime.mcpEnabledTools` instead of using them to build native per-tool wrappers. This avoids duplicating registry-resolution/JSON-parsing logic across 5 engine constructors, which would violate single-responsibility (the resolution logic already has exactly one owner) for no behavioral benefit — `ExecutionParams` was never going to lose these fields' *data*, only their *native-injection consumer*.

**Alternative considered (original D7)**: Remove the fields from `ExecutionParams`, inject `McpRegistryPool` into all 5 engine constructors, duplicate pool resolution + `enabled_mcp_tools` parsing in each engine's context-construction code. Rejected — this is a substantially larger, more invasive change (touches `index.ts`'s `engineFactories` plus 5 engine constructors) that spreads a single concern (registry/filter resolution) across every engine, contradicting this session's SOLID/loose-coupling directive, for a purely cosmetic reduction in `ExecutionParams`'s field count.
