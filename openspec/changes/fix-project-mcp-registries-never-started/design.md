...
- **integration** (backend-rpc-runtime harness, in-memory DB): `support/shared-rpc-scenarios.ts` gains `runProjectScopedMcpDiscoveryScenario` — it writes a `.railyn/mcp.json` into the workspace config's project dir, then drives the real `ExecutionParamsBuilder.build()` → `getForProject(projectPath)` → executor path (via `copilot-rpc-scenarios.test.ts` with a per-scope fresh registry pool); asserts `list_mcp_servers` reports `running` (never `idle`) and `invoke_mcp_tool` succeeds — the `grafana-mcpar` shape. Note: the `mcp.getStatus` RPC is global-only, so the project-scoped path can only be observed through the task-execution harness, not via an RPC.
