...
- [x] 7.5 Update `config/engines.yaml.sample` to the `thinking:` + `options.reasoning_effort` variant shape. User's ~/.railyn/config/engines.yaml requires manual migration (none→`thinking:false`+`reasoning_effort:none`, normal→`thinking:true`+`reasoning_effort:high`, max→`thinking:true`+`reasoning_effort:max`).
