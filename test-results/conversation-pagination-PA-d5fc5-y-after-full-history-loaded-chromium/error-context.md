...
- Name: conversation-pagination.spec.ts >> PAG-8 — sentinel hidden when all history loaded >> PAG-8: loading spinner gone and sentinel empty after full history loaded
- Location: e2e/ui/conversation-pagination.spec.ts:235:5
...
Call log:
  ...
  - waiting for locator('.conv-body .msg').first()
    9 × locator resolved to <div data-v-4e81c418="" data-v-1a6653d5="" class="msg msg--assistant">…</div>
...
```ts
  ...
  181 |     test.skip("PAG-6: older paged history is preserved when stream done fires", async ({ page, api, ws, task }) => {
