...
export async function openTaskDrawer(taskId: number): Promise<void> {
  ...
  // Step 2: set activeTaskId, load messages, open drawer — fire-and-forget.
  await webEval(`
    var pinia = document.querySelector('#app').__vue_app__.config.globalProperties['$pinia'];
    var taskStore = pinia._s.get('task');
    var drawerStore = pinia._s.get('drawer');
    taskStore.activeTaskId = ${taskId};
    taskStore.loadMessages(${taskId}); // fire-and-forget
    // Find task to get conversationId
  ...
  const opened = await waitFor(".task-chat-view", 8_000);
...
export interface ChatSessionBasic {
  id: number;
  workspaceKey: string;
  title: string;
  status: string;
...
/**
 * Seed a chat session via the debug server (creates DB row + pushes chatSession.created).
 */
export async function seedChatSession(opts: { workspaceKey?: string; title?: string } = {}): Promise<ChatSessionBasic> {
  const res = await fetch(`${BRIDGE_BASE}/seed-chat-session`, {
    method: "POST",
    body: JSON.stringify(opts),
    headers: { "content-type": "application/json" },
  });
  const data = await res.json() as ChatSessionBasic & { __error?: string };
  if (data.__error) throw new Error(`seedChatSession failed: ${data.__error}`);
  return data;
}

/**
 * Push a chatSession.updated event to the frontend without touching the DB.
 */
export async function pushChatSessionUpdated(session: Partial<ChatSessionBasic> & { id: number }): Promise<void> {
  const res = await fetch(`${BRIDGE_BASE}/push-chat-session-updated`, {
    method: "POST",
    body: JSON.stringify(session),
    headers: { "content-type": "application/json" },
  });
  const data = await res.json() as { ok?: boolean; __error?: string };
  if (data.__error) throw new Error(`pushChatSessionUpdated failed: ${data.__error}`);
}

/** Return all data-session-id values visible in the sidebar, in DOM order. */
export async function getSessionIds(): Promise<number[]> {
  return webEval<number[]>(`
    return Array.from(document.querySelectorAll('[data-session-id]'))
      .map(el => Number(el.getAttribute('data-session-id')));
  `);
}

/** Return the status-dot class for a given session item (e.g. "status-dot--running"). */
export async function getSessionStatusClass(sessionId: number): Promise<string> {
  return webEval<string>(`
    var el = document.querySelector('[data-session-id="${sessionId}"] .session-item__status-dot');
    if (!el) return '';
    return Array.from(el.classList).find(c => c.startsWith('status-dot--')) || '';
  `);
}

/** Return true if the session item has the has-unread CSS class. */
export async function sessionHasUnread(sessionId: number): Promise<boolean> {
  return webEval<boolean>(`
    var el = document.querySelector('[data-session-id="${sessionId}"]');
    return el ? el.classList.contains('has-unread') : false;
  `);
}

/** Click a session item in the sidebar to open the session panel (now ConversationDrawer). */
export async function openSessionPanel(sessionId: number): Promise<void> {
  await webClick(`[data-session-id="${sessionId}"]`);
  // Wait for unified drawer to mount with session content
  const visible = await waitFor(".session-chat-view", 5_000);
  if (!visible) throw new Error(`openSessionPanel: drawer did not appear for session ${sessionId}`);
}

/** Return true if the session drawer is currently visible. */
export async function isSessionPanelVisible(): Promise<boolean> {
  return webEval<boolean>(`return !!document.querySelector('.session-chat-view')`);
}

/** Return the title shown in the session drawer header. */
export async function getSessionPanelTitle(): Promise<string> {
  return webEval<string>(`
    var el = document.querySelector('.scv-header__title');
    return el ? el.textContent?.trim() ?? '' : '';
  `);
}

/** Return the number of messages with the given role in the active session drawer. */
export async function getSessionMessageCount(role: "user" | "assistant"): Promise<number> {
  return webEval<number>(`
    return document.querySelectorAll('.session-chat-view .msg--${role}').length;
  `);
}

/** Close the session drawer by clicking the archive button (which closes the drawer). */
export async function closeSessionPanel(): Promise<void> {
  // Use Pinia to close directly — avoids needing a dedicated close button
  await webEval(`
    var pinia = document.querySelector('#app').__vue_app__.config.globalProperties['$pinia'];
    pinia._s.get('chat').closeSession();
    return 'ok';
  `);
  await sleep(400);
}
