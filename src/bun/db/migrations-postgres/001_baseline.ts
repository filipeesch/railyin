...
const STATEMENTS: string[] = [
  ...
  `CREATE TABLE task_line_comments (
     id            integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     task_id       integer NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
     file_path     text NOT NULL,
  ...
  `CREATE TABLE conversation_injection_state (
  ...
  `CREATE INDEX idx_tasks_board_state ON tasks(board_id, workflow_state)`,
  ...
  `CREATE INDEX idx_executions_task_status_tokens ON executions(task_id, status, input_tokens)`,
  `CREATE INDEX idx_logs_task ON logs(task_id)`,
  `CREATE INDEX idx_logs_level ON logs(level)`,
  `CREATE INDEX idx_logs_execution ON logs(execution_id)`,
  `CREATE INDEX idx_logs_created ON logs(created_at)`,
  `CREATE INDEX idx_hunk_decisions_task ON task_hunk_decisions(task_id)`,
  `CREATE INDEX idx_line_comments_task_file_sent ON task_line_comments(task_id, file_path, sent)`,
  `CREATE INDEX idx_pending_messages_task ON pending_messages(task_id)`,
  `CREATE INDEX idx_task_todos_task ON task_todos(task_id)`,
  `CREATE INDEX idx_task_notes_conversation ON task_notes(conversation_id)`,
  `CREATE INDEX idx_stream_events_conversation ON stream_events(conversation_id, seq)`,
  `CREATE INDEX idx_stream_events_execution ON stream_events(execution_id, seq)`,
  `CREATE INDEX idx_stream_events_conv_exec_seq ON stream_events(conversation_id, execution_id, seq)`,
  `CREATE INDEX idx_chat_sessions_workspace_activity ON chat_sessions(workspace_key, last_activity_at DESC)`,
  `CREATE INDEX idx_model_raw_messages_execution_seq ON model_raw_messages(execution_id, stream_seq)`,
  `CREATE INDEX idx_model_raw_messages_task_created ON model_raw_messages(task_id, created_at)`,
  `CREATE INDEX idx_model_raw_messages_engine_type ON model_raw_messages(engine, event_type)`,
  `CREATE INDEX idx_decision_records_conversation_deleted ON decision_records(conversation_id, is_deleted)`,
  `CREATE INDEX idx_decision_revisions_decision_id ON decision_revisions(decision_id)`,
];
