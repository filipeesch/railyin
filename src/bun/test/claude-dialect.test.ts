...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "claude-dialect-test-"));
  dialect = new ClaudeDialect();
});
...
function writeCommandFile(relativePath: string, content: string, dir = tmpDir): void {
  const commandDir = join(dir, ".claude", "commands");
...
describe("ClaudeDialect.resolvePrompt — non-slash", () => {
...
describe("ClaudeDialect.resolvePrompt — slash commands", () => {
...
describe("ClaudeDialect.resolvePrompt — path priority", () => {
  ...
  it("resolves from projectPath when command only exists there", async () => {
    const projectDir = mkdtempSync(join(tmpdir(), "claude-project-"));
    try {
...
describe("ClaudeDialect.listCommands", () => {
  it("returns an array even when no .claude/commands dir exists", () => {
    const commands = dialect.listCommands(tmpDir);
    // May include real home-scope commands; at minimum it should be an array
    expect(Array.isArray(commands)).toBe(true);
  });

  it("returns commands from worktreePath/.claude/commands", () => {
