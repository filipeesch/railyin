...
async function createClaudeRuntime(adapter: MockClaudeSdkAdapter): Promise<BackendRpcRuntime> {
  adapter.setModels([
    {
...
describe("Claude backend RPC scenarios", () => {
  it("covers single-turn and multi-turn chat via shared scenarios", async () => {
    ...
    const runtime = await createClaudeRuntime(adapter);
  ...
  it("surfaces shell approval pauses and resumes after approval", async () => {
    ...
    expect((await runtime.getMessages(taskId)).some((message) => message.type === "ask_user_prompt" && message.content.includes('"subtype":"shell_approval"'))).toBe(true);
...
describe("Claude engine — subagent scenarios", () => {
  ...
  it("CRS-SA-2: subagent Bash with unapproved binary emits shell_approval and resumes on approval", async () => {
    ...
    expect((await runtime.getMessages(taskId)).some((m) => m.type === "ask_user_prompt" && m.content.includes('"subtype":"shell_approval"'))).toBe(true);
  ...
  it("CRS-SA-3: subagent_start is persisted as tool_call conversation message with subagentId", async () => {
    ...
    const dbEvents = await runtime.getDbStreamEvents(executionId);
