...
describe("Test Plan: Validate search_text removal and SDK search tool replacement", () => {
  ...
  it("DEFAULT_PI_TOOL_GROUPS has 4 entries: read, write, shell, web", () => {
    expect(DEFAULT_PI_TOOL_GROUPS).toEqual(["read", "write", "shell", "web"]);
  });
...
// buildChildTools() was removed when the delegate tool was disabled on Pi engine.
// These tests are no longer relevant — child session tool surface is managed by the delegate tool.
