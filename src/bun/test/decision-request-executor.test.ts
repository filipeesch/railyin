...
describe("executeDecisionRequest", () => {
  it("appends a valid question and returns a page result with count + end-turn hint", () => {
    const ctx = makeCtx();
    const result = executeDecisionRequest(
      { question: { question: "Which DB?", type: "exclusive", options: [{ title: "PG", description: "Postgres" }, { title: "SQLite", description: "SQLite embedded" }] } },
      ctx,
    );
  ...
  it("returns error for exclusive question with fewer than 2 options (no buffer mutation)", () => {
    const ctx = makeCtx();
    const result = executeDecisionRequest(
      { question: { question: "Which DB?", type: "exclusive", options: [{ title: "PG", description: "Postgres" }] } },
      ctx,
    );
  ...
  it("returns error when missing type (runtime options-count guard still covers non-freetext)", () => {
    ...
    const result = executeDecisionRequest({ question: { question: "Pick?" } }, ctx);
    expect(result.type).toBe("result");
    expect(ctx.runtime.decisionBuffer!.count).toBe(0);
  });
  ...
  it("appends freetext question without options", () => {
    const ctx = makeCtx();
    const result = executeDecisionRequest({ question: { question: "Any constraints?", type: "freetext" } }, ctx);
    expect(result.type).toBe("page");
    expect(ctx.runtime.decisionBuffer!.count).toBe(1);
  });
  ...
  it("returns error when no buffer is available", () => {
    ...
    const result = executeDecisionRequest({ question: { question: "Q", type: "freetext" } }, ctx);
    expect(result.type).toBe("result");
  ...
  it("returns count reflecting accumulation across calls", () => {
    const ctx = makeCtx();
    executeDecisionRequest({ question: { question: "Q1", type: "freetext" } }, ctx);
    const second = executeDecisionRequest({ question: { question: "Q2", type: "freetext" } }, ctx);
    expect((second as { text: string }).text).toContain("Question 2 of 2 buffered");
  });

  it("folds per-question context into the page payload", () => {
    const ctx = makeCtx();
    const result = executeDecisionRequest({ context: "Preamble", question: { question: "Q1", type: "freetext" } }, ctx);
    expect(result.type).toBe("page");
