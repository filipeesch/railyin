...
- **Pi reasoning config (breaking)**: The per-model `models.<id>.interleaved` field is **renamed** to `models.<id>.thinkingFormat` (values `openai|openrouter|deepseek|together|zai|qwen|chat-template|qwen-chat-template|string-thinking|ant-ling`), mapping to the pi SDK's `compat.thinkingFormat`; `validatePiEngineConfig` rejects a legacy `interleaved` key. `reasoning` maps to `model.reasoning`. Note: `thinkingFormat: deepseek` also sets `compat.requiresReasoningContentOnAssistantMessages`. Variants drive the SDK through a canonical `thinkingLevel` derived from each variant's `reasoningEffort`; the SDK owns the `reasoning_effort` key, so Railyin never emits it via `onPayload` (provider-specific knobs like `enable_thinking`/`chat_template_kwargs` may still pass through). `tool_call` is accepted-but-informational for now.
- **Pi API mode (breaking default)**: `engines.yaml` Pi engines accept `api: openai-completions | openai-responses` (engine root) with an optional `providers.<name>.api` override; resolution is `providers.<name>.api ?? config.api ?? "openai-responses"`. The **default flipped to `openai-responses`** (`/v1/responses`) — existing configs without `api:` must add `api: openai-completions` to keep `/v1/chat/completions`. `thinkingFormat` is a completions-only compat knob and is silently dropped under responses. Session auth uses the SDK's `ModelRuntime` (removed `AuthStorage`); session creation is consolidated in `createPiAgentSession` (`pi-session-factory.ts`) with `buildPiModelRuntime` (`model-runtime-builder.ts`) registering providers + api modes.
...
- **Startup maintenance** (`StartupMaintenance` in `src/bun/db/startup-maintenance.ts`) writes a consistent `VACUUM INTO` snapshot to `<db>.backup` on **every** boot and reclaims free space with a threshold-gated `VACUUM` (free pages > 64 MB or > 10% of the file) — deleted conversations shrink the file automatically at the next start. It is best-effort: a locked DB (another instance) logs a warning and boot continues.
